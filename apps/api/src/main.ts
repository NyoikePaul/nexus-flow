...
async function bootstrap() {
  ...
  app.enableCors({ origin:origins, methods:['GET','POST','PATCH','DELETE','OPTIONS'], credentials:true });
  ...
  if (process.env.NODE_ENV !== 'production' || process.env.ENABLE_SWAGGER === 'true') {
    const doc = SwaggerModule.createDocument(app, new DocumentBuilder()
      .setTitle('NexusFlow API').setDescription('Enterprise logistics workflow engine').setVersion('1.0')
      .addTag('shipments').addTag('health').addBearerAuth().build());
